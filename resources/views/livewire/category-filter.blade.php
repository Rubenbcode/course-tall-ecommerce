    ...
    <div class="bg-white rounded-lg shadow-lg mb-6">
        <div class="px-6 py-2 flex justify-between items-center">
            ...
            <div class="grid grid-cols-2 border border-gray-200 divide-x divide-gray-200">
                <i 
                class="fas fa-border-all p-3 text-gray-500 cursor-pointer
                {{$view == 'grid' ? 'text-orange-500' : ''}} "
                wire:click="$set('view','grid')"></i>
                <i 
                class="fas fa-th-list p-3 text-gray-500 cursor-pointer
                {{$view == 'list' ? 'text-orange-500' : ''}}"
        ...
        <aside>
            ...
            <ul class="divide-y divide-gray-200">
                @foreach($category->subcategories as $subcategory)
                    <li class="py-2 text-sm">
                        <a
                        wire:click="$set('subcategoryset','{{$subcategory->name}}')"
                        class="cursor-pointer hover:text-orange-500 capitalize {{$subcategoryset == $subcategory->name ? 'text-orange-500 font-semibold' : ''}}">{{$subcategory->name}}</a>
                    </li>
            ...
            <ul class="divide-y divide-gray-200">
                @foreach($category->brands as $brand)
                    <li class="py-2 text-sm">
                        <a
                        wire:click="$set('brandset','{{$brand->name}}')"

                         class="cursor-pointer hover:text-orange-500 capitalize {{$brandset == $brand->name ? 'text-orange-500 font-semibold' : ''}}">{{$brand->name}}</a>
                    </li>
                            ...
                            <div class="flex-1 py-4 px-6 flex flex-col">
                                <div class="flex justify-between">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-gray-700 text-sm">(25)</span>
                                    </div>
